package com.newbee.material.repo.util.tag;
...
public class PagerTag extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		if (recordCount > 0) { // 1499 / 15 = 100
			...
			/** 拼接其它的信息 */
			res.append(
					"<table width='100%' align='center' style='font-size:13px;' class='"
							+ style + "'>");
			res.append(
			...
			res.append("总共<font color='red'>" + this.recordCount
					+ "</font>条记录，当前显示" + startNum + "-" + endNum + "条记录。");
			res.append("</td></tr>");
			res.append("</table>");
			...
			res.append(
					"      if (!/^[1-9]\\d*$/.test(page_size) || page_size < 1 || page_size > "
							+ this.totalPage + "){");
			res.append(
	...
	private void calcPage(StringBuilder str) {
		...
		if (this.totalPage <= 11) {
			...
			if (this.pageIndex <= 8) {
				...
				str.append(
						"<a href='" + tempUrl + "'>" + this.totalPage + "</a>");
			}
			/** 靠近尾页 */
